...
export default function ListingDetailPage() {
      ...
      try {
        ...
        const message =
          (err as { response?: { data?: { errors?: string[] } } })?.response
            ?.data?.errors?.[0] ?? "Failed to send inquiry";
        toast.error(message);
      }
    },
  });
  ...
  const reviewFormik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        ...
        const message =
          (err as { response?: { data?: { errors?: string[] } } })?.response
            ?.data?.errors?.[0] ?? "Failed to submit review";
        toast.error(message);
      }
    },
  });
  ...
  const canInquire =
    isAuthenticated && user?.role === "Buyer" && listing.sellerId !== user?.id;
  const hasAlreadyReviewed =
    reviews?.reviews.some((r) => r.reviewerId === user?.id) ?? false;
  const canReview =
    isAuthenticated &&
    user?.role === "Buyer" &&
    !hasAlreadyReviewed &&
              ...
              <div className="flex gap-2 flex-wrap">
                {listing.images.map((img, i) => (
                  <button
                    ...
                    className={`relative h-16 w-16 rounded-xl overflow-hidden border-2 transition-all ${activeImage === i
          ...
          <div className="space-y-5">
            <div>
              ...
              <div className="mt-2 flex items-center gap-2 flex-wrap">
                ...
                <Badge variant={listing.gender === "Male" ? "info" : "rose"}>
                  {listing.gender}
                </Badge>
            ...
            {canInquire && inquirySent && (
              <div className="rounded-xl bg-green-50 border border-green-200 p-4 text-center">
                ...
                <p className="text-sm font-medium text-green-700">
                  Inquiry sent! Check your inquiries page for replies.
                </p>
              </div>
            )}
            {!isAuthenticated && (
            ...
            <form onSubmit={reviewFormik.handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 text-right mt-0.5">
                  {reviewFormik.values.comment.length}/500
                </p>
              </div>

              <Button
                type="submit"
                isLoading={reviewFormik.isSubmitting}
                disabled={reviewFormik.values.rating === 0}
              >
                Submit Review
              </Button>
            </form>
        ...
        <form onSubmit={inquiryFormik.handleSubmit} className="space-y-4">
          ...
          <div>
            <textarea
              ...
              rows={4}
              className={`w-full rounded-xl border px-4 py-3 text-sm outline-none focus:ring-2 focus:border-transparent resize-none ${inquiryFormik.touched.initialMessage &&
                  inquiryFormik.errors.initialMessage
                  ? "border-red-400 bg-red-50 focus:ring-red-300"
                  : "border-gray-200 focus:ring-rose-400"
                }`}
            />
            {inquiryFormik.touched.initialMessage &&
              inquiryFormik.errors.initialMessage && (
                <p className="mt-1 text-xs text-red-500">
                  {inquiryFormik.errors.initialMessage}
                </p>
            ...
            <p className="text-xs text-gray-400 text-right mt-0.5">
              {inquiryFormik.values.initialMessage.length}/500
            </p>
          </div>
          <div className="flex gap-3">
            ...
            <Button
              type="submit"
              isLoading={inquiryFormik.isSubmitting}
              className="flex-1"
            >
              Send Message
            </Button>

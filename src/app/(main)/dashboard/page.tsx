...
import { Listing, ListingStatus, PagedResult } from '@/types';
import { formatPrice, formatDate, formatAge } from '@/lib/utils/format';
import { Plus, Edit2, Trash2, Send, Upload, Eye, Star, X, ShieldCheck } from 'lucide-react';
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="col-span-2">
          ...
          <textarea name="description" value={formik.values.description} onChange={formik.handleChange} onBlur={formik.handleBlur}
            ...
            className={`w-full rounded-xl border px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-rose-400 resize-none ${
              formik.touched.description && formik.errors.description ? 'border-red-400 bg-red-50' : 'border-gray-200'
            }`} />
...
function ImageModal({
  ...
  const upload = async () => {
    ...
    if (sizeMB > 5) { toast.error('File must be under 5MB'); return; }
    setUploading(true);
    try {
      ...
      <Modal isOpen={!!imageListing} onClose={() => setImageListing(null)} title="Manage Images" maxWidth="md">
        {imageListing && (
          <ImageModal
            ...
            onRefresh={async () => {
              await load(page);
              // re-sync imageListing with fresh data
